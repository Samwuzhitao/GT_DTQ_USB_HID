...
class voice_monitor(QThread):
    ...
    def get_update_file_name(self):
        ...
        self.file_name = "DTQ_%08X_VOICE_%s.mp3" % (self.devid, voice_time)
...
class XesHT46Pro():
    ...
    def dec_to_bcd(self, dec_data):
        bcd_data = ((((dec_data / 10) % 10) & 0x0F) << 4) | ((dec_data % 10) & 0x0F)
        return bcd_data
    ...
    def get_cur_bcd_time_arr(self):
        ...
        time_arr.append(tmp)
        # 月
        time_arr.append(self.dec_to_bcd(cur_time.month))
        # 日
        time_arr.append(self.dec_to_bcd(cur_time.day))
        # 时
        time_arr.append(self.dec_to_bcd(cur_time.hour))
        # 分
        time_arr.append(self.dec_to_bcd(cur_time.minute))
        # 秒
        time_arr.append(self.dec_to_bcd(cur_time.second))
        # 毫秒
        tmp = self.dec_to_bcd(cur_time.microsecond / 10000)
        time_arr.append(tmp)
    ...
    def get_arr_time_str(self, arr):
        time_str = "%02x%02x-%02x-%02x %02x:%02x:%02x,%02x%02x" % (arr[0], arr[1], arr[2], arr[3], arr[4], arr[5], arr[6], arr[7], arr[8])
        return time_str
    ...
    '''
    # 白名单监测
    def devid_check(self, uid_arr):
        tmpid = self.get_dec_uid(uid_arr)
        if tmpid in self.dtqdict:
            return True
        else:
            return False
