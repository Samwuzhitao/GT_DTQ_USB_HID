...
class DtqUsbHidDebuger(QWidget):
    ...
    def usb_hid_send_data(self):
        i = 0
        msg = [0x01, 0x01, 0x01, 0x13, 0x00, 0x12]
        msg.insert(0, 0x00)
        datalen = len(msg)
        for i in range(65):
            if i >= datalen:
                msg.append(0x00)
        if self.report:
            self.report[0].set_raw_data(msg)
            self.report[0].send()
            msg = self.usbhidmonitor.cmd_decode.list_export(msg)
            self.browser.append(u"S : {0}".format(msg))
            logging.debug(u"发送数据：S : {0}".format(msg))
